...
這篇把這 2 小時稽核攤開：怎麼用黑箱視角偵察、5 個步驟怎麼跑、為什麼有些發現要在交報告前降級、跟我刻意不做的事。
...
紙面上看這些限制很綁手綁腳，實際上它們強制這份稽核的工作方式：沒有 Studio、沒有 service role，等於只剩外部能觀察到的訊號可用，跟一個真實外部攻擊者的視角一樣。每個發現都要從可觀察的行為「推論」架構問題，不是從管理員視角直接讀出來。

這也是這份稽核要回答的問題：「目前開出去的存取權限，被人能拿來做什麼？」，不是「資料庫 schema 在管理員視角下長什麼樣？」
...
「任何登入用戶可簽任何 path + signed URL 一天有效」這個組合，是這份稽核影響範圍最大的發現。
...
暴露路徑還在。攻擊者可以把 key 嵌到一個自己控制、且 referer header 滿足限制的 domain 裡，從瀏覽器發起呼叫。如果 referer 白名單設錯 (用 `*.example.com` 之類萬用字元)，暴露範圍會擴大。如果未來改 config 把 referer 限制移掉，暴露立刻變可利用。

但這些都是條件性、二階風險，不是「下一小時內輪換」的等級。比較合適的分類是 Medium，配兩條具體建議：
...
這是兩種完全不同的對話。「現在立刻輪換 + 稽核所有 client app」是一種，「移除多餘暴露 + 確認 referer 設定正確」是另一種。兩種都會讓系統更安全，但只有後者把工程時間跟營運緊急度當稀缺資源看待。

重點就在這：發現只是起點，再驗證一次，才寫得出對的建議。

多數自動掃描工具跟跑清單式的稽核在「發現」就停了，因為發現是可觀察的；判斷需要驗證加上一次真實世界的測試。這也是客戶自己不容易做的部分，因此也是客戶會付錢買的部分。
...
- 一份 scanner output 貼進附錄
- 「Looks concerning」的發現但沒有驗證
- 為了讓案子看起來值錢全面拉嚴重度
- 建議跑掉變成架構大改 (「實作 zero-trust storage policies」)
- 在實際存在不確定性的地方用絕對化語氣 (「系統對 X 攻擊脆弱」)
- 計時器超過 2 小時上限還繼續跑

我交的版本沒做這些：

- 24 小時 signed URL expiry 是觀察值，跟 path-guess 發現配對。沒另起一條，因為它改變的是 path-guess 發現的影響範圍，所以併在同一條 Medium 裡。
- 錯誤訊息洩漏資訊是真的，但是 Low。不會為了讓報告看起來內容多就拉到 Medium。
- 計時器停在 1:51 / 2:00。報告寫作在那之後做的，off-clock，沒算進客戶的 2 小時 budget。合約字面講的是 2 小時稽核，按合約精神交付的是「客戶能拿去用的報告」。
- 正式環境狀態沒在報告裡描述。稽核是 staging-only，每個對正式環境的描述前面都有「在範圍內無法驗證」這個前綴，因為這就是事實。
- HIGH → Medium 降級沒被淡化處理。原始觀察、驗證步驟、重新分類的邏輯全寫進報告，讓客戶能跟著我的發現一起 audit 我的判斷。

「報告裡刻意不寫什麼」，就是用心稽核跟自動工具輸出最大的差別。
...
- **建議**：具體下一步動作，不是主題。「在簽發階段強制 path 層級的 project ownership」是具體；「強化 storage 安全姿態」不是。

對「單一最高優先建議」這項，pattern 是：一個建議，不是清單。為什麼選這一條的理由要寫進去 (影響範圍、目前沒有緩解、不會牽動其他部分的修法)。這條之後的下一步順序也要點出來 (storage hardening 出貨後再去處理 `google-maps-key` cleanup)。

這是編輯工作跟技術工作同等重要。客戶讀完交付物，應該不只知道「找到了什麼」，還知道「先做什麼後做什麼，以及為什麼」。
...
判斷工作不一樣。「對 HIGH 發現多花 5 分鐘驗證」的決定。「不為了報告好看膨脹 Low」的決定。「稽核是 staging-only 所以結論不講正式環境」的決定。「建議寫成可執行步驟而不是主題」的決定。沒有一個是掃描工具的輸出，沒有一個是清單的項目。這些是讀過夠多稽核報告 (好的跟爛的) 才知道你自己這份正在變成哪種。
